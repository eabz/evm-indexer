...
--     optional: toString(FixedString) and CAST to String TRIM TRAILING
--     ZERO BYTES, so a pubkey ending in 0x00 routed through either would
--     print as a different key (dex::api v4). base58Encode(id) itself
--     does NOT trim on 25.12.1.322 - see the header of migration 0006 for
--     the measured values - but the substring() form is the one to copy:
--     it states the intent and holds on every build.
